# polyglot inside Pika

How polyglot becomes a tab in Pika: shared student, matched art style, lesson
...
| Question | Answer |
| --- | --- |
| Integration shape | Minimal port between two services, Pal-style. polyglot stays its own deploy. |
| Identity | Pika owns it outright. polyglot keeps no account system on this path — no login, no password, no join code. |
| First slice | Full: lesson results reach Pika's gradebook as assignment grades. |
| Art style | Theme boundary. Pika supplies semantic tokens, polyglot keeps its layout. |
...
| Half | Owner | Notes |
| --- | --- | --- |
| polyglot API, token verification, identity linking, results feed | us | Fully unblocked. |
| Theme bridge variables consumed by polyglot | us | We define the names; Pika supplies values. |
...
## How polyglot appears inside Pika
...
**polyglot is not widget-shaped, and that is the whole answer.** Pal's widget is
a panel — an achievements card, a companion sprite — which drops into a content
pane happily. polyglot is a three-column application: lesson list, lesson,
...
| Surface | Shape | Why |
| --- | --- | --- |
| A `lessons` tab | The whole polyglot app, filling the content pane | It needs its own shell. Nothing smaller preserves it. |
...
Pika's rail plus the polyglot sidebar and top bar puts two navigations on screen
...
| | What is on screen |
| --- | --- |
| **The tab** | Pika's own cards — units, progress, "Continue: 6.4". No polyglot chrome at all, because it is not polyglot; it is Pika showing where you are. |
| **The lesson** | polyglot with the viewport to itself. Pika's rail is gone, and so is our top bar. One slim strip: back to the classroom, and progress. |
...
On ours, **the polyglot top bar stays and the way back goes into it.** An earlier
...
Signing out of polyglot from inside a Pika tab would leave a student in a tab
...
cookie OR a Pika bearer token. polyglot inside a frame simply never uses a
...
- **Port polyglot into the pika repo.** Two framework majors, a data layer and an
...
than a rebuild: `rewrites` in Pika pointing `/java/*` at polyglot, plus
...
The teacher registers polyglot once and gets a shared `secret`. polyglot stores it
...
polyglot stores that hash on `User` as `pikaSubject` (unique). The raw Pika user
id is never persisted.

### 3. Read token — Pika mints, polyglot verifies
...
polyglot verifies signature, `aud`, `iss` and `exp` on every request. A token is
a read credential for one student, nothing more.

### 3a. The owner stays an admin here, whatever Pika says

Worth stating because it is easy to assume otherwise: **polyglot roles are not
derived from Pika.**

A Pika student is resolved to a polyglot `User` row and given `role: "STUDENT"`.
...
polyglot has **no account system** on this path. No login, no password, no join
...
point at. polyglot never authenticates against it.

**No linking, no backfill, no migration.** Every account currently on polyglot is
...
**Pika pulls; polyglot does not push.** Pal's direction is the opposite because
Pika is the signal *source* there. Here polyglot is the source, and pulling keeps
polyglot from ever holding write credentials into Supabase. Smaller blast radius,
smaller port.
...
Pika supplies a narrow set of semantic variables; polyglot maps its palette onto
...
object and no raw student id, and Pika owns the container while polyglot owns
its contents.

**Code is never restyled.** Snippets, the runner and program output stay in the
polyglot mono treatment whatever Pika's tokens say. A student reading a compiler
...
polyglot runs in two shapes and must keep doing so.
