...
async function runJava(code: string, stdin = "", mode: WrapMode = "beginner", append = ""): Promise<Run> {
  const wrapped = wrapAs(code, mode);
  // Hidden library classes go AFTER the wrapper closes, as sibling top-level
  // classes. Folding them in with the student's code nests them inside main().
  const source = append ? `${wrapped.source}\n\n${append}` : wrapped.source;
  let lastErr = "";
...
async function checkStep(s: FlowStep): Promise<string[]> {
  ...
  const expect = async (code: string, stdin: string, want: string, what: string) => {
    const r = await runJava(code, stdin, s.wrap || "beginner", s.library || "");
  ...
  switch (s.kind) {
    ...
    case "run": { const r = await runJava(s.code!, s.stdin || "", s.wrap || "beginner", s.library || ""); if (!r.compiled || r.error) fails.push(`${at}: does not run clean — ${(r.error || "").split("\n")[0].slice(0, 110)}`); break; }
    case "tweak": await expect(s.code!, s.stdin || "", s.target!, "starting output"); break;
    case "ask": { const stdin = (s.fields || []).map((f) => f.sample).join("\n");
      const r = await runJava(s.code!, stdin, s.wrap || "beginner", s.library || ""); if (!r.compiled || r.error) fails.push(`${at}: does not run with the sample answers — ${(r.error || "").split("\n")[0].slice(0, 110)}`); break; }
    case "predict": { const want = s.opts![s.correct!];
      if (/error|crash/i.test(want)) { const r = await runJava(s.code!, s.stdin || "", s.wrap || "beginner", s.library || ""); if (r.compiled && !r.error) fails.push(`${at}: claims an error but it runs fine`); }
