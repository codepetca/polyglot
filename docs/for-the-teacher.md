# classOS + Pika — what I'm asking for

Short version: **classOS becomes a tab in Pika. Pika owns the accounts. I keep
...
"continue where you left off". No classOS chrome at all; it reads as part of
Pika because it is.

Clicking a lesson hands the whole screen to classOS. Your rail goes away, my
...
- Student identity. classOS has **no account system** on this path — no login,
...
- A results feed at `GET /api/pika/v1/student/results`, so your gradebook can
  read lesson scores whenever it wants. classOS never writes into Supabase and
...
A classOS lesson is a **learning item**. I can send `learning_item.viewed` and
...
**I'll keep shipping.** classOS stays its own repo and its own deploy, so I can
...
**I need to stay an admin in classOS.** I'll be in senior form, not in this
class, so "teacher of this classroom" won't describe me. classOS roles aren't
...
And it isn't a one-way door: if the frame turns out to be wrong, serving classOS
